...
-(void) initDBData:(POSDataSet*) dataSet
{
    if(sqlite3_open([dataPath UTF8String], &database) == SQLITE_OK)
    {
        ...
        if(n == 0)
        {
            ...
            if(sqlite3_exec(database, [query UTF8String], NULL, NULL, &errorMsg) != SQLITE_OK)
            {
                NSAssert(0, @"Error inserting into collection: %s", errorMsg);
                statusDB = NO;
            }
        }
        
        //query = @"SELECT id FROM collection WHERE name=\"Nissan\"";
        ...
        if(n == 0)
        {
            ...
            if(sqlite3_exec(database, [query UTF8String], NULL, NULL, &errorMsg) != SQLITE_OK)
            {
                NSAssert(0, @"Error inserting into product: %s", errorMsg);
                statusDB = NO;
            }
        ...
        if(n == 0)
        {
            ...
            if(sqlite3_exec(database, [query UTF8String], NULL, NULL, &errorMsg) != SQLITE_OK)
            {
                NSAssert(0, @"Error inserting into image: %s", errorMsg);
                statusDB = NO;
            }
